...
rollDice().then((value) => console.log(`Success! Dice settled on ${value}.`)).catch((err) => console.log(err.message));
/****** problems mentioned in readme file still not exist because with promises when resolve or reject called once (only one call to which occurs first), it blocks other following calls ******/
